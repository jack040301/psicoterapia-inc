                ...
                <table style="width:100%; border-collapse:collapse; margin:25px 0; font-size:0.9em; border-radius:5px 5px 0 0; overflow: hidden;">
                    ...
                    <tbody>
                        <?php
                           ...
                           // $get_data_query = "SELECT *, DATEDIFF(date(now()), birthday)/365.25 as age FROM tbl_appts WHERE date < date(now()) ORDER BY id DESC LIMIT $offset, $total_records_per_page";
                           
                           $get_data_query = "SELECT *,t2.status FROM tbl_useraccount as t1 left join tbl_appts as t2 on t1.userID = t2.userID left join tbl_employee as t3 on t3.id = t2.docID WHERE date < date(now()) ORDER BY id DESC LIMIT $offset, $total_records_per_page";
