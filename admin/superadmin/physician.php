    ...
    <body onload="display_ct();">
        ...
        <div class="container-fluid row">
            ...
            <section class="home-section">
                ...
                <a href="team.php" style="text-decoration:none; font-size:10pt; padding:10px; background-color: rgba(203, 209, 231, .3); border-top: 2px solid #5065AF;">Moderators</a>
                <a href="physician.php" style="text-decoration:none; font-size:10pt; padding:10px; background-color: rgba(203, 209, 231, .7); border-top: 2px solid #5065AF; text-shadow: 1px 1px 1px #fff;">Doctors</a>
                <table style="width:100%; border-collapse:collapse; margin:25px 0; font-size:0.9em; border-radius:5px 5px 0 0; overflow: hidden;">
                    ...
                    <tbody>
                        <?php
	                        ...
	                        $result_count = mysqli_query($connect,"SELECT COUNT(*) As total_records FROM tbl_employee WHERE position = 'doctor'");
                            ...
                            $get_data_query = "SELECT *, DATEDIFF(date(now()), birthday)/365.25 as age FROM tbl_employee WHERE position = 'doctor' ORDER BY id ASC LIMIT $offset, $total_records_per_page";
                            $get_data = mysqli_query($connect, $get_data_query);
                            for($i=0; $row = mysqli_fetch_array($get_data); $i++){
                        ?>
                            <tr style="border-bottom:2px solid whitesmoke;"><td><?php echo $row['id']; ?></td><td><?php echo ucwords($row['lastname']); ?></td><td><?php echo ucwords($row['firstname']); ?></td><td><?php echo $row['email']; ?></td><td><?php echo number_format($row['age']); ?></td><td><?php echo $row['created']; ?></td><td><?php echo ucwords($row['status']); ?></td><td style="max-width:70px;"><a href="employee.php?employee_id=<?=$row['id']?>"><i style="font-size:12pt; color:#5065AF;" class='bx bx-show-alt'></i></a> <a href="../data/control.php?doctor_id=<?=$row['id']?>" style="text-decoration: none;"><?php if($row['status'] == 'online'){ echo 'Deactivate';}else{echo 'Activate';}?></a></td></tr>
                        <?php } ?>
                    </tbody>
                </table>
