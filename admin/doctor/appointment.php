                ...
                <table style="width:100%; border-collapse:collapse; margin:25px 0; font-size:0.9em; border-radius:5px 5px 0 0; overflow: hidden;">
                    ...
                    <tbody>
                        <?php
	                        ...
	                        $result_count = mysqli_query($connect,"SELECT COUNT(*) As total_records FROM tbl_appointments where date = date(now()) AND doctor = '$email' AND status = 'pending'");
                            ...
                            $get_data_query = "SELECT *, DATEDIFF(date(now()), birthday)/365.25 as age FROM tbl_appointments WHERE date = date(now()) AND doctor = '$email' AND status = 'pending' ORDER BY time ASC LIMIT $offset, $total_records_per_page";
                        ...
                        ?>
                            <tr style="border-bottom:2px solid whitesmoke;"><td><?php echo $row['id']; ?></td><td><?php echo $row['time']; ?></td><td><?php echo ucwords($row['lastname']); ?></td><td><?php echo ucwords($row['firstname']); ?></td><td><?php echo number_format($row['age']); ?></td><td><?php echo $row['mobile']; ?></td><td><?php echo ucwords($row['status']); ?></td><td><a href="transac.php?client_id=<?=$row['email']?>" style="font-size: 9pt; width: 80px;" class="btn btn-primary">Transac</a></td></tr>
                        <?php } ?>
                    </tbody>
                </table>
