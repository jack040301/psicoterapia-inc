    ...
    <body onload="display_ct();">
        ...
        <div class="container-fluid row">
            ...
            <section class="home-section">
                ...
                <a href="upcoming.php" style="text-decoration:none; font-size:10pt; padding:10px; background-color: rgba(203, 209, 231, .3); border-top: 2px solid #5065AF;">Upcoming Appointments</a>
                <a href="past.php" style="text-decoration:none; font-size:10pt; padding:10px; background-color: rgba(203, 209, 231, .3); border-top: 2px solid #5065AF;">Past's Appointments</a>
                <table style="width:100%; border-collapse:collapse; margin:25px 0; font-size:0.9em; border-radius:5px 5px 0 0; overflow: hidden;">
                    <thead style="background-color:#F1F3FF; text-align:left;">
                        <tr><th>Ref. #</th><th>Time</th><th>Lastname</th><th>Firstname</th><th>Age</th><th>Contact No.</th><th>Status</th><th>Action</th></tr>
                    </thead>
                    <tbody>
                        <?php
	                ...
	                //        $result_count = mysqli_query($connect,"SELECT COUNT(*) As total_records FROM tbl_appts LEFT JOIN tbl_employee on (tbl_appts.docID = tbl_employee.id) where (date = date(now()) AND email = '$email' AND tbl_appts.status = 'pending'");
                            ...
                            $result_count = mysqli_query($connect,"SELECT date, count(*) as total_records from tbl_appts left join tbl_employee on tbl_appts.docID = tbl_employee.id WHERE date <= date(now()) AND email = '$doc_email' AND tbl_appts.status = 'pending'");
                        //  $result_count = ($connect, "SELECT date, count(*) as total_records from tbl_appts left join tbl_employee on tbl_appts.docID = tbl_employee.id WHERE date <= date(now()) AND email = '$doc_email' GROUP BY date ORDER BY date ASC'");
                          ...
                          $get_data_query = "SELECT *,t2.status FROM tbl_useraccount as t1 left join tbl_appts as t2 on t1.userID = t2.userID left join tbl_employee as t3 on t3.id = t2.docID WHERE date = date(now()) AND email = '$email' AND t2.status = 'pending'  ORDER BY t2.time ASC LIMIT $offset, $total_records_per_page ";
                        ...
                        ?>
                            <tr style="border-bottom:2px solid whitesmoke;"><td><?php echo $row['id']; ?></td><td><?php echo $row['time']; ?></td><td><?php echo ucwords($row['userSurname']); ?></td><td><?php echo ucwords($row['userGivenName']); ?></td><td><?php echo number_format($row['userAge']); ?></td><td><?php echo $row['userContactNumber']; ?></td><td><?php echo ucwords($row['status']); ?></td><td><a href="transac.php?client_id=<?=$row['userEmail']?>" style="font-size: 9pt; width: 80px;" class="btn btn-primary">Transac</a></td></tr>
                        <?php } ?>
                    </tbody>
                </table>
