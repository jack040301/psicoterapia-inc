...
<html>
    ...
    <head>
        ...
        <title><?php echo ucwords($fetch_company_details['companyname1']); ?><?php echo strtolower($fetch_company_details['companyname2']); ?> | Upcoming Appointments</title>
        <meta charset="utf-8">
    ...
    <body onload="display_ct();">
        ...
        <div class="container-fluid row">
            ...
            <section class="home-section">
                ...
                <a href="appointment.php" style="text-decoration:none; font-size:10pt; padding:10px; background-color: rgba(203, 209, 231, .3); border-top: 2px solid #5065AF; ">Today's Appointments</a>
                <a href="upcoming.php" style="text-decoration:none; font-size:10pt; padding:10px; background-color: rgba(203, 209, 231, .7); border-top: 2px solid #5065AF; text-shadow: 1px 1px 1px #fff;">Upcoming Appointments</a>
                ...
                <table style="width:100%; border-collapse:collapse; margin:25px 0; font-size:0.9em; border-radius:5px 5px 0 0; overflow: hidden;">
                    <thead style="background-color:#F1F3FF; text-align:left;">
                        <tr><th>Ref. #</th><th>Date</th><th>Time</th><th>Lastname</th><th>Firstname</th><th>Age</th><th>Contact No.</th><th>Status</th></tr>
                    </thead>
                    <tbody>
                        <?php
                            ...
                            $get_data_query = "SELECT *,t2.status FROM tbl_useraccount as t1 left join tbl_appts as t2 on t1.userID = t2.userID left join tbl_employee as t3 on t3.id = t2.docID WHERE date > date(now()) AND email = '$email' ORDER BY t3.id ASC LIMIT $offset, $total_records_per_page";
                        ...
                        ?>
                            <tr style="border-bottom:2px solid whitesmoke;"><td><?php echo $row['apptID']; ?></td><td><?php echo $row['date']; ?></td><td><?php echo $row['time']; ?></td><td><?php echo ucwords($row['userSurname']); ?></td><td><?php echo ucwords($row['userGivenName']); ?></td><td><?php echo number_format($row['userAge']); ?></td><td><?php echo $row['userContactNumber']; ?></td><td><?php echo ucwords($row['status']); ?></td></tr>
                        <?php } ?>
                    </tbody>
                </table>
