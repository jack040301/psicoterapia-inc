...
if ($_SERVER['REQUEST_METHOD'] == "POST") {
	...
	//
		...
		// $pick = $_POST['datepick'];
		// $doc = $_POST['displaydoctor'];
		// $time = $_POST['displaytime'];	
		// $appointmentNumber = "SELECT * FROM tbl_appointment where (select max(userNumber))";
		// $check_number = mysqli_query($conn, $appointmentNumber);
		// if( mysqli_num_rows($check_number) > 0){
		// 	$fetch_number = mysqli_fetch_assoc($check_number);
		// 	$maxNumber = $fetch_number['userNumber'];
		...
		$sqlinsertappoinment = "INSERT INTO tbl_appointment VALUES ($maxnum,'{$_SESSION['userID']}','$datepick', '$displaydoctor', '$displaytime','$today')";
